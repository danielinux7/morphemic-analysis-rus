...
while True:
    ...
    if word_or_text == 'с':
        ...
        if search != 'д':
            ...
            type_morph = input('Введите тип морфа: ')
            morph = input('Введите морф: ')
            search_title = '{} "{}" встречается в данном тексте в следующих словоформах:\n'.format(type_morph, morph)
            search_res = search_title
            type_morphs = {}
            for el in information:
                for ind, element in enumerate(el[2]):
                    if element == type_morph and el[1][ind] == morph:
                        search_res += ' | '.join([el[0], '/'.join(el[1]), '/'.join(el[2])]) + '\n'
